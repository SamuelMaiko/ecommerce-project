...
const OrderSummary = () => {
  const { cartNumber, cartTotal } = useMyGeneralContext();
  return (
    <div className="bg-[rgba(0,0,0,0.03)] shadow-inner w-[26rem] p-10 pt-14 h-full">
      ...
      <div className="my-8">
        <label className="uppercase font-body font-medium text-sm">
          Shipping address
        </label>
        <input
      ...
      <div>
        <p className="uppercase font-medium font-body text-sm">
          Items {cartNumber}
        </p>
        <p>Ksh. {cartTotal}</p>
      </div>
      ...
      <button className="mt-4 w-full uppercase shadow-[2px_2px_10px_rgba(0,0,0,0.1)] bg-[#ff9900] hover:bg-[#ff8029] transition-colors duration-300 text-white font-button border-none rounded-[1px] px-[20px] py-[10px] text-sm">
        Checkout
      </button>
    </div>
  );
